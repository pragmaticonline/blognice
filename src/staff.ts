...
async function accountById(c: any, id: number) {
  ...
  try {
    return await c.env.DB.prepare(
      `SELECT a.id, a.email, COALESCE(a.status, 'active') AS status, a.status_reason, a.status_changed_at, a.created_at, a.stripe_customer_id, a.billing_status, a.billing_price_id, a.billing_period_end, a.billing_cancel_at_period_end, a.api_key_hash IS NOT NULL AS has_api_key, COALESCE(a.email_verified,0) AS email_verified, a.email_verified_at, a.signup_ip, a.signup_ua, a.signup_referer, a.signup_country, a.locked_until, a.deleted_at, a.vip_granted_at, a.vip_expires_at, a.vip_granted_by, a.vip_reason, a.max_blogs_override, (SELECT COUNT(*) FROM sessions s WHERE s.account_id = a.id AND s.expires_at > ?) AS active_sessions, (SELECT COUNT(*) FROM memberships m WHERE m.account_id = a.id) AS blog_count FROM accounts a WHERE a.id = ?`
    ).bind(now, id).first();
  } catch {
    return await c.env.DB.prepare(
...
function billingPlan(account: any, c: any): string {
  const vipActive = Number(account.vip_granted_at || 0) > 0 && (account.vip_expires_at == null || Number(account.vip_expires_at) > Math.floor(Date.now()/1000));
...
app.get("/staff-favicon.svg", (c) => new Response(staffFaviconSvg, { headers: { "content-type": "image/svg+xml", "cache-control": "public, max-age=3600" } }));
...
app.post("/api/accounts/:id/vip", async (c) => {
  const staff = c.get("staff") as StaffIdentity; if (!canMutate(staff)) return c.json({ error: "staff role cannot manage VIP" }, 403);
  if (!sameOrigin(c)) return c.json({ error: "same-origin request required" }, 403);
  const id = Number(c.req.param("id")); if (!Number.isSafeInteger(id) || id < 1) return c.json({ error: "invalid account" }, 400);
  const body = await c.req.json().catch(() => ({})) as any;
  const action = String(body.action || "").trim().toLowerCase();
  const reason = String(body.reason || "").trim().slice(0, 500);
  const expiresInDaysRaw = body.expiresInDays ?? body.expires_in_days ?? null;
  const now = Math.floor(Date.now() / 1000);
  const before: any = await accountById(c, id); if (!before) return c.json({ error: "account not found" }, 404);
  if (action === "revoke") {
    if (!reason) return c.json({ error: "a reason is required" }, 400);
    try { await c.env.DB.prepare("UPDATE accounts SET vip_granted_at=NULL, vip_expires_at=NULL, vip_granted_by=NULL, vip_reason=? WHERE id=?").bind(reason, id).run(); } catch (e) { return c.json({ error: "VIP columns not available; apply migration 057" }, 500); }
    await audit(c, staff, { action: "vip-revoke", targetType: "account", targetId: String(id), reason, result: "success", before: { vip_granted_at: before.vip_granted_at, vip_expires_at: before.vip_expires_at }, after: { vip_granted_at: null } });
    return c.json({ ok: true, account: await accountById(c, id) });
  }
  if (action === "grant") {
    if (!reason) return c.json({ error: "a reason is required" }, 400);
    let expiresAt: number | null = null;
    if (expiresInDaysRaw != null && String(expiresInDaysRaw).trim() !== "") {
      const days = Number(expiresInDaysRaw);
      if (!Number.isFinite(days) || days < 1 || days > 3650) return c.json({ error: "expiresInDays must be 1-3650" }, 400);
      expiresAt = now + Math.floor(days * 86400);
    }
    try { await c.env.DB.prepare("UPDATE accounts SET vip_granted_at=?, vip_expires_at=?, vip_granted_by=NULL, vip_reason=? WHERE id=?").bind(now, expiresAt, reason, id).run(); } catch (e) { return c.json({ error: "VIP columns not available; apply migration 057" }, 500); }
    await audit(c, staff, { action: "vip-grant", targetType: "account", targetId: String(id), reason, result: "success", before: { vip_granted_at: before.vip_granted_at }, after: { vip_granted_at: now, vip_expires_at: expiresAt } });
    return c.json({ ok: true, account: await accountById(c, id) });
  }
  return c.json({ error: "action must be grant or revoke" }, 400);
});

app.post("/api/accounts/:id/max-blogs", async (c) => {
  const staff = c.get("staff") as StaffIdentity; if (!canMutate(staff)) return c.json({ error: "staff role cannot manage blog limits" }, 403);
  if (!sameOrigin(c)) return c.json({ error: "same-origin request required" }, 403);
  const id = Number(c.req.param("id")); if (!Number.isSafeInteger(id) || id < 1) return c.json({ error: "invalid account" }, 400);
  const body = await c.req.json().catch(() => ({})) as any;
  const reason = String(body.reason || "").trim().slice(0, 500);
  const raw = body.max_blogs ?? body.maxBlogs ?? body.value;
  const before: any = await accountById(c, id); if (!before) return c.json({ error: "account not found" }, 404);
  ...
  if (raw == null || String(raw).trim() === "") {
    ...
    const n = Number(raw);
    if (!Number.isSafeInteger(n) || n < 1 || n > 50) return c.json({ error: "max_blogs must be 1-50 or empty to clear" }, 400);
    override = n;
  }
  if (!reason) return c.json({ error: "a reason is required" }, 400);
  try { await c.env.DB.prepare("UPDATE accounts SET max_blogs_override=? WHERE id=?").bind(override, id).run(); } catch (e) { return c.json({ error: "max_blogs_override column not available; apply migration 057" }, 500); }
  await audit(c, staff, { action: "max-blogs-override", targetType: "account", targetId: String(id), reason, result: "success", before: { max_blogs_override: before.max_blogs_override }, after: { max_blogs_override: override } });
  return c.json({ ok: true, account: await accountById(c, id) });
});
...
app.post("/api/tts-test", async (c) => {
  ...
  try {
    ...
    await audit(c, staff, { action: "tts-test", targetType: "tts", targetId: model, reason: "Generate short pronunciation sample", result: "failure", after: { characters: text.length, error: info.category, code: info.code, transient: info.transient, retries: Array.isArray(retries) ? retries : [] } });
...
app.get("/staff/experiments/affiliate-offer", async (c) => {
  ...
  const trendStatus = trendError
    ? `<p class="alert"><strong>Trend data unavailable.</strong> Verify the staff Worker Analytics token and its Account Analytics Read permission.</p>`
    : !trends.length ? `<p class="muted">No trend events have been recorded yet.</p>` : "";
...
app.get("/accounts/:id", async (c) => {
  ...
  const isVipActive = Number(account.vip_granted_at || 0) > 0 && (account.vip_expires_at == null || Number(account.vip_expires_at) > Math.floor(Date.now()/1000));
  const vipExpiryLabel = account.vip_expires_at ? new Date(Number(account.vip_expires_at) * 1000).toISOString().slice(0,10) : "lifetime";
  const vipCard = `<div class="card"><h2>VIP</h2>${isVipActive ? `<p><span class="badge">VIP active</span> Granted ${new Date(Number(account.vip_granted_at)*1000).toISOString().slice(0,10)} · Expires ${esc(vipExpiryLabel)}${account.vip_reason ? ` · ${esc(account.vip_reason)}` : ""}</p>` : `<p class="muted">No VIP — account uses normal Free/Pro entitlement.</p>`}${canMutate(staff) ? `<div class="actions" style="flex-direction:column;align-items:flex-start"><form data-vip-action="/api/accounts/${id}/vip" style="display:flex;gap:8px;flex-wrap:wrap;align-items:center"><input name="reason" required placeholder="Reason for VIP" style="min-width:200px"><input name="expiresInDays" type="number" min="1" max="3650" placeholder="Days (empty=lifetime)" style="width:160px"><button class="btn" type="submit" data-vip="grant">Grant VIP</button><button class="btn btn-danger" type="button" data-vip="revoke">Revoke VIP</button></form></div><script>(function(){var f=document.querySelector('form[data-vip-action="/api/accounts/${id}/vip"]');if(!f)return;f.querySelector('[data-vip="grant"]').addEventListener('click',async function(e){e.preventDefault();var r=f.elements.reason.value;if(!r){alert('Reason required');return}var d=f.elements.expiresInDays.value;var body=JSON.stringify({action:'grant',reason:r,expiresInDays:d||null});var res=await fetch(f.dataset.vipAction,{method:'POST',headers:{'content-type':'application/json'},body:body});var j=await res.json();if(!res.ok){alert(j.error||'Failed');return}location.reload();});f.querySelector('[data-vip="revoke"]').addEventListener('click',async function(e){e.preventDefault();var r=f.elements.reason.value;if(!r){alert('Reason required');return}var body=JSON.stringify({action:'revoke',reason:r});var res=await fetch(f.dataset.vipAction,{method:'POST',headers:{'content-type':'application/json'},body:body});var j=await res.json();if(!res.ok){alert(j.error||'Failed');return}location.reload();});})();</script>` : ""}</div>`;
  const currentMax = (account as any).max_blogs_override;
  const blogsLimitCard = `<div class="card"><h2>Blog limit</h2><p><strong>Max blogs:</strong> ${currentMax != null ? esc(String(currentMax)) + ' (override)' : (isVipActive || ["active","trialing","past_due"].includes(billingStatus) ? '5 (Pro/VIP default)' : '1 (Free default)')} · Owned: ${account.blog_count}</p>${canMutate(staff) ? `<div class="actions"><form data-blogs-action="/api/accounts/${id}/max-blogs" style="display:flex;gap:8px;flex-wrap:wrap"><input name="max_blogs" type="number" min="1" max="50" placeholder="1-50 (empty=clear)" value="${currentMax != null ? esc(String(currentMax)) : ""}" style="width:130px"><input name="reason" required placeholder="Reason" style="min-width:180px"><button class="btn" type="submit">Save limit</button><button class="btn" type="button" data-clear>Clear override</button></form></div><script>(function(){var f=document.querySelector('form[data-blogs-action="/api/accounts/${id}/max-blogs"]');if(!f)return;f.addEventListener('submit',async function(e){e.preventDefault();var v=f.elements.max_blogs.value;var r=f.elements.reason.value;if(!r){alert('Reason required');return}var body=JSON.stringify({max_blogs:v===""?null:Number(v),reason:r});var res=await fetch(f.dataset.blogsAction,{method:'POST',headers:{'content-type':'application/json'},body:body});var j=await res.json();if(!res.ok){alert(j.error||'Failed');return}location.reload();});var c=f.querySelector('[data-clear]');if(c)c.addEventListener('click',async function(){var r=f.elements.reason.value;if(!r){alert('Reason required to clear');return}var res=await fetch(f.dataset.blogsAction,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({max_blogs:null,reason:r})});var j=await res.json();if(!res.ok){alert(j.error||'Failed');return}location.reload();});})();</script>` : `<p class="muted">Read-only role.</p>`}</div>`;
  ...
  const autopilotCard = autopilotCardHtml(blogs.results as any[]);
...
function autopilotCardHtml(blogs: any[]): string {
  const ids = (blogs as any[]).map((b:any)=> (b as any).tenant_id).filter((v:any)=> Number.isSafeInteger(v));
  const idsJson = JSON.stringify(ids);
  const blogsJson = JSON.stringify((blogs as any[]).map((b:any)=> ({ id:(b as any).tenant_id, title:(b as any).title, slug:(b as any).slug, custom_domain:(b as any).custom_domain })))
  return `<div class="card"><div class="card-head"><h2>Autopilot</h2><a class="btn" href="/autopilot-runs">View runs →</a></div><p class="muted">Staff toggle (admin, Pro/VIP + add-on ready, max 1/day). Stored as <code>autopilot_configs</code> + <code>autopilot_runs</code>. Topic 3-120 chars, Brave Search 1 req/post.</p><table><thead><tr><th>Blog</th><th>Status / Topic</th><th>Schedule</th><th>Actions</th></tr></thead><tbody id="autopilot-rows"><tr><td colspan="4" class="empty">Loading…</td></tr></tbody></table><div id="autopilot-edit-panel" style="display:none;margin-top:16px;padding:16px;border:1px solid var(--rule);border-radius:9px;background:var(--paper)"><h3 style="margin:0 0 8px">Autopilot setup</h3><p class="muted">Topic required 3-120. Advanced optional: include/exclude ≤10×40, freshness 24h/7d/30d, domains ≤20, tone neutral/concise/friendly/authoritative, audience 0-120, tags ≤5, dedup 7-90.</p><form id="autopilot-form" style="display:grid;gap:10px"><input type="hidden" name="tenant_id"><label>Topic * <input name="topic" required minlength="3" maxlength="120" placeholder="e.g. daily AI tools and startups" style="width:100%;padding:8px;border:1px solid var(--rule);border-radius:5px"></label><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px"><label>Interval days <input name="interval_days" type="number" min="1" max="7" value="1" style="width:100%;padding:8px;border:1px solid var(--rule);border-radius:5px"></label><label>Run hour UTC <input name="run_hour_utc" type="number" min="0" max="23" value="9" style="width:100%;padding:8px;border:1px solid var(--rule);border-radius:5px"></label><label>Max length <input name="max_length" type="number" min="400" max="2000" value="900" style="width:100%;padding:8px;border:1px solid var(--rule);border-radius:5px"></label></div><details><summary>Advanced criteria (optional)</summary><div style="display:grid;gap:8px;margin-top:10px"><label>Include (comma, ≤10) <input name="include" placeholder="ai, startup" style="width:100%;padding:8px;border:1px solid var(--rule);border-radius:5px"></label><label>Exclude (comma, ≤10) <input name="exclude" placeholder="crypto, politics" style="width:100%;padding:8px;border:1px solid var(--rule);border-radius:5px"></label><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px"><label>Freshness <select name="freshness" style="width:100%;padding:8px;border:1px solid var(--rule);border-radius:5px"><option value="7d">7d</option><option value="24h">24h</option><option value="30d">30d</option></select></label><label>Tone <select name="tone" style="width:100%;padding:8px;border:1px solid var(--rule);border-radius:5px"><option value="neutral">neutral</option><option value="concise">concise</option><option value="friendly">friendly</option><option value="authoritative">authoritative</option></select></label></div><label>Allow domains (comma, ≤20) <input name="allowDomains" placeholder="example.com, techcrunch.com" style="width:100%;padding:8px;border:1px solid var(--rule);border-radius:5px"></label><label>Block domains (comma, ≤20) <input name="blockDomains" placeholder="spam.com" style="width:100%;padding:8px;border:1px solid var(--rule);border-radius:5px"></label><label>Audience (0-120) <input name="audience" maxlength="120" placeholder="developers and founders" style="width:100%;padding:8px;border:1px solid var(--rule);border-radius:5px"></label><label>Tags (comma, ≤5) <input name="tags" placeholder="ai, news" style="width:100%;padding:8px;border:1px solid var(--rule);border-radius:5px"></label><label>Dedup days (7-90) <input name="dedup_days" type="number" min="7" max="90" value="30" style="width:100%;padding:8px;border:1px solid var(--rule);border-radius:5px"></label></div></details><label>Reason * (10-500) <input name="reason" required minlength="10" maxlength="500" placeholder="e.g. enable per owner request #123" style="width:100%;padding:8px;border:1px solid var(--rule);border-radius:5px"></label><div style="display:flex;gap:8px"><button class="btn" type="submit">Enable / Save</button><button class="btn btn-danger" type="button" id="autopilot-disable" style="display:none">Disable</button><button class="btn" type="button" id="autopilot-cancel">Cancel</button></div></form></div><script>(function(){var tids=${idsJson};var blogs=${blogsJson};var tbody=document.getElementById('autopilot-rows');var panel=document.getElementById('autopilot-edit-panel');var form=document.getElementById('autopilot-form');var cancel=document.getElementById('autopilot-cancel');function esc(s){var d=document.createElement('div');d.textContent=s;return d.innerHTML}function csv(s){return String(s||'').split(',').map(function(x){return x.trim()}).filter(Boolean)}function load(){if(!tids.length){tbody.innerHTML='<tr><td colspan=4 class=empty>No blogs.</td></tr>';return}Promise.all(tids.map(function(id){return fetch('/api/autopilot/'+id).then(function(r){return r.json().then(function(j){return {id:id, ok:r.ok, cfg:j.config||null}})}).catch(function(){return {id:id, ok:false}})} )).then(function(results){var map=new Map(results.map(function(x){return [x.id, x.cfg]}));var html='';blogs.forEach(function(blog){var tid=blog.id;var cfg=map.get(tid);var crit=(cfg&&cfg.criteria)||{};var enabled=cfg && Number(cfg.enabled) && Number(cfg.staff_enabled);var needTopic=!enabled&&!crit.topic;var status=enabled?'<span class=badge>Autopilot ON</span>':'<span class=badge suspended>Off</span>'+(needTopic?' <span class="badge warn">Needs topic</span>':'');var topic=crit.topic?esc(crit.topic):'<span class=muted>—</span>';var next=cfg&&cfg.next_run_at?new Date(Number(cfg.next_run_at)*1000).toISOString().replace('T',' ').slice(0,19)+' UTC':'<span class=muted>—</span>';var host=blog.custom_domain|| (blog.slug+'.'+(document.location.hostname.split('.').slice(-2).join('.')||'blognice.com'));html+='<tr><td>'+esc(blog.title)+'<br><small>#'+tid+' · '+esc(host)+'</small></td><td>'+status+'<br><small>'+topic+'</small></td><td><small>interval '+(cfg?cfg.interval_days:1)+'d · hour '+(cfg?cfg.run_hour_utc:9)+' UTC<br>max '+(cfg?cfg.max_length:900)+' · next '+next+'</small></td><td><button class=btn data-autopilot-toggle='+tid+' data-enabled='+(enabled?0:1)+'>'+(enabled?'Disable':(needTopic?'Set up':'Enable'))+'</button> <button class=btn data-autopilot-edit='+tid+'>Edit</button> <button class=btn data-autopilot-run='+tid+'>Run now</button></td></tr>';});tbody.innerHTML=html||'<tr><td colspan=4 class=empty>No blogs.</td></tr>';bind()});}function openPanel(tid){form.elements.tenant_id.value=tid;fetch('/api/autopilot/'+tid).then(function(r){return r.json()}).then(function(j){var cfg=j.config||{};var crit=cfg.criteria||{};var on=cfg&&Number(cfg.enabled)&&Number(cfg.staff_enabled);form.elements.topic.value=crit.topic||'';form.elements.interval_days.value=cfg.interval_days||1;form.elements.run_hour_utc.value=cfg.run_hour_utc||9;form.elements.max_length.value=cfg.max_length||900;form.elements.freshness.value=crit.freshness||'7d';form.elements.tone.value=crit.tone||'neutral';form.elements.audience.value=crit.audience||'';form.elements.dedup_days.value=crit.dedup_days||30;form.elements.include.value=(crit.include||[]).join(', ');form.elements.exclude.value=(crit.exclude||[]).join(', ');form.elements.allowDomains.value=(crit.allowDomains||[]).join(', ');form.elements.blockDomains.value=(crit.blockDomains||[]).join(', ');form.elements.tags.value=(crit.tags||[]).join(', ');var dis=document.getElementById('autopilot-disable');if(dis)dis.style.display=on?'':'none';});panel.style.display='block';panel.scrollIntoView({behavior:'smooth'})}
function bind(){document.querySelectorAll('[data-autopilot-edit]').forEach(function(b){b.addEventListener('click',function(){openPanel(b.getAttribute('data-autopilot-edit'))})});document.querySelectorAll('[data-autopilot-toggle]').forEach(function(b){b.addEventListener('click',function(){openPanel(b.getAttribute('data-autopilot-toggle'))})});document.querySelectorAll('[data-autopilot-run]').forEach(function(b){b.addEventListener('click',async function(){var tid=b.getAttribute('data-autopilot-run');if(!confirm('Run autopilot now for blog #'+tid+'? This may publish a post.'))return;b.disabled=true;b.innerHTML='<span class=spin></span>Running…';try{var r=await fetch('/api/autopilot/'+tid+'/run-now',{method:'POST',headers:{'content-type':'application/json'},body:'{}'});var j=await r.json();if(!r.ok){alert(j.error||'Failed');b.disabled=false;b.textContent='Run now';return}if(j.started){var since=Number(j.since)||Math.floor(Date.now()/1000);var deadline=Date.now()+300000;var run=null;while(Date.now()<deadline){await new Promise(function(res){setTimeout(res,5000)});try{var lr=await fetch('/api/autopilot-runs?tenant='+tid+'&limit=5');var lj=await lr.json();var rows=lj.runs||[];for(var i=0;i<rows.length;i++){if(Number(rows[i].started_at)>=since-5&&rows[i].finished_at!=null){run=rows[i];break}}}catch(e){}if(run)break}if(!run){alert('Run is still going — check View runs for the result.');location.reload();return}}else{var run=j.run||{}}alert('Run finished: '+(run.status||'unknown')+' '+(run.error||''));}catch(e){alert('Failed')}location.reload()})});}if(cancel)cancel.addEventListener('click',function(){panel.style.display='none'});if(form)form.addEventListener('submit',async function(e){e.preventDefault();var tid=form.elements.tenant_id.value;if(!tid){alert('Pick a blog via Edit first');return}var payload={enabled:true,topic:form.elements.topic.value.trim(),interval_days:Number(form.elements.interval_days.value),run_hour_utc:Number(form.elements.run_hour_utc.value),max_length:Number(form.elements.max_length.value),reason:form.elements.reason.value.trim(),freshness:form.elements.freshness.value,tone:form.elements.tone.value,audience:form.elements.audience.value.trim(),dedup_days:Number(form.elements.dedup_days.value),include:csv(form.elements.include.value),exclude:csv(form.elements.exclude.value),allowDomains:csv(form.elements.allowDomains.value),blockDomains:csv(form.elements.blockDomains.value),tags:csv(form.elements.tags.value)};if(!payload.topic||payload.topic.length<3){alert('Topic 3-120 required');return}if(!payload.reason||payload.reason.length<10){alert('Reason 10-500 required');return}var sub=form.querySelector('button[type=submit]');if(sub){sub.disabled=true;sub.innerHTML='<span class=spin></span>Working…'}var r=await fetch('/api/autopilot/'+tid+'/toggle',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});var j=await r.json();if(!r.ok){alert(j.error||'Failed');if(sub){sub.disabled=false;sub.textContent='Enable / Save'}return}location.reload()});var disBtn=document.getElementById('autopilot-disable');if(disBtn)disBtn.addEventListener('click',async function(){var tid=form.elements.tenant_id.value;var reason=form.elements.reason.value.trim();if(!reason||reason.length<10){alert('Reason 10-500 required');return}var r=await fetch('/api/autopilot/'+tid+'/toggle',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({enabled:false,reason:reason})});var j=await r.json();if(!r.ok){alert(j.error||'Failed');return}location.reload()});load()})();<\/script></div>`;
}
...
app.post("/api/autopilot/:tenantId/toggle", async (c) => {
  ...
  const existingForCriteria = await c.env.DB.prepare("SELECT criteria_json FROM autopilot_configs WHERE tenant_id=?").bind(tenantId).first() as any;
  let existingCriteria: any = {};
  try { existingCriteria = JSON.parse(existingForCriteria?.criteria_json || "{}"); } catch { existingCriteria = {}; }
  ...
  if (body.include !== undefined) {
    ...
    include = (body.include as any[]).map((s:any)=>String(s).trim()).filter(Boolean);
    for (const kw of include!) if (kw.length < 1 || kw.length > 40) return c.json({ error: "each include keyword must be 1-40 characters" }, 400);
  } else if (Array.isArray(existingCriteria.include)) include = existingCriteria.include;
  ...
  if (body.exclude !== undefined) {
    ...
    exclude = (body.exclude as any[]).map((s:any)=>String(s).trim()).filter(Boolean);
    for (const kw of exclude!) if (kw.length < 1 || kw.length > 40) return c.json({ error: "each exclude keyword must be 1-40 characters" }, 400);
  } else if (Array.isArray(existingCriteria.exclude)) exclude = existingCriteria.exclude;
  let freshness = body.freshness !== undefined ? String(body.freshness).trim() : existingCriteria.freshness;
  ...
  if (body.allowDomains !== undefined) {
    ...
    allowDomains = (body.allowDomains as any[]).map((s:any)=>String(s).trim().toLowerCase()).filter(Boolean);
    for (const d of allowDomains!) if (d.length > 253 || !/^([a-z0-9-]+\.)+[a-z]{2,}$/.test(d)) return c.json({ error: `invalid allowDomains entry: ${d}` }, 400);
  } else if (Array.isArray(existingCriteria.allowDomains)) allowDomains = existingCriteria.allowDomains;
  ...
  if (body.blockDomains !== undefined) {
    ...
    blockDomains = (body.blockDomains as any[]).map((s:any)=>String(s).trim().toLowerCase()).filter(Boolean);
    for (const d of blockDomains!) if (d.length > 253 || !/^([a-z0-9-]+\.)+[a-z]{2,}$/.test(d)) return c.json({ error: `invalid blockDomains entry: ${d}` }, 400);
  } else if (Array.isArray(existingCriteria.blockDomains)) blockDomains = existingCriteria.blockDomains;
  let tone = body.tone !== undefined ? String(body.tone).trim() : existingCriteria.tone;
  ...
  let audience = body.audience !== undefined ? String(body.audience).trim() : existingCriteria.audience;
  ...
  if (body.tags !== undefined) {
    ...
    tags = (body.tags as any[]).map((s:any)=>String(s).trim()).filter(Boolean);
    for (const tg of tags!) if (tg.length < 1 || tg.length > 40) return c.json({ error: "each tag must be 1-40 characters" }, 400);
  } else if (Array.isArray(existingCriteria.tags)) tags = existingCriteria.tags;
  let dedup_days: number | undefined = body.dedup_days !== undefined ? Number(body.dedup_days) : existingCriteria.dedup_days;
...
app.post("/api/autopilot/:tenantId/run-now", async (c) => {
  const staff = c.get("staff");
  if (!staff) return c.json({ error: "staff authorization required" }, 403);
  if (!canAdmin(staff)) return c.json({ error: "admin role required for autopilot" }, 403);
  if (!sameOrigin(c)) return c.json({ error: "same-origin request required" }, 403);
...
app.get("/autopilot-runs", async (c) => {
  ...
  const rows = (results || []).map((r: any) => {
    ...
    const image = r.image_status
      ...
      : "—";
    const startedSec = Number(r.started_at);
