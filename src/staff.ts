...
function staffPage(title: string, body: string): string {
  ...
  </style></head><body><main class="wrap">${body}</div></div></main><footer class="staff-footer"><span><strong>blognice</strong> · © 2026 Pragmatic Online Co., Ltd.</span><nav aria-label="Legal"><a href="https://www.blognice.com/policies">Policies</a></nav></footer><script>(function(){var path=location.pathname,hash=location.hash;function setActive(){document.querySelectorAll('[data-staff-nav]').forEach(function(link){link.classList.remove('active');var href=link.getAttribute('href')||'';var parts=href.split('#'),target=parts[0]||'/',anchor=parts[1]||'';var isEmail=target==='/'&&anchor==='email-preview'&&location.hash==='#email-preview';var isAccounts=target==='/'&&!isEmail&&location.hash!=='#email-preview'&&(location.pathname==='/'||location.pathname.indexOf('/accounts/')===0);if(isEmail||isAccounts||(target!=='/'&&location.pathname.indexOf(target)===0))link.classList.add('active');});}setActive();window.addEventListener('hashchange',setActive);var toggle=document.querySelector('.staff-menu-toggle'),sidebar=document.getElementById('staff-sidebar');if(toggle&&sidebar){toggle.addEventListener('click',function(){var open=sidebar.classList.toggle('open');toggle.setAttribute('aria-expanded',String(open));});document.addEventListener('click',function(event){if(sidebar.classList.contains('open')&&!sidebar.contains(event.target)&&event.target!==toggle){sidebar.classList.remove('open');toggle.setAttribute('aria-expanded','false');}});document.addEventListener('keydown',function(event){if(event.key==='Escape'&&sidebar.classList.contains('open')){sidebar.classList.remove('open');toggle.setAttribute('aria-expanded','false');toggle.focus();}});}})();</script></body></html>`;
}
