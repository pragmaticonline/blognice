...
export function subscriberWelcomeEmail(input: { blogTitle: string; unsubscribeUrl: string; manageUrl: string }) {
  ...
  return {
    ...
    html: `<div style="text-align:center"><h2 style="font-family:Arial,sans-serif;margin:0 0 14px;color:#181a12">You're subscribed</h2><p>Thanks for subscribing to <strong>${title}</strong>. You'll get new posts by email.</p></div><hr><p style="text-align:center;color:#5c6455;font-size:12px"><a href="${unsub}" style="color:#5c6455">Unsubscribe</a> anytime · <a href="${manage}" style="color:#5c6455">Manage subscriptions</a></p>`,
  };
}
...
function withIdentityFooter(msg: EmailMessage): EmailMessage {
  ...
  const htmlFooter = `<div style="background:#f7f8f5;padding:22px 32px;text-align:center;color:#5c6455;font-size:12px;line-height:1.7">${bulk ? `Sent by ${senderHtml} via blognice.<br>` : ""}<span>Support: ${PLATFORM_SUPPORT}</span> · <a href="${PLATFORM_PRIVACY}" style="color:#5c6455">Privacy</a> · <a href="${PLATFORM_TERMS}" style="color:#5c6455">Terms</a> · <a href="${PLATFORM_SECURITY}" style="color:#5c6455">Security</a>${bulk ? `<br><br>${PLATFORM_POSTAL}` : ""}</div>`;
