...
export function metricsPage(
  ...
  if (options?.configured === false) {
    ...
    <div class="metrics-grid">
      ...
      <div class="panel-block"><h2 style="margin-top:0;font-size:1rem">Subscribers</h2><div style="display:flex;flex-wrap:wrap;gap:1.2rem;margin-bottom:.7rem"><span><strong>${report.subscribers.emailSubscribed.toLocaleString()}</strong> email subscribed</span><span><strong>${report.subscribers.emailUnsubscribed.toLocaleString()}</strong> unsubscribed</span><span><strong>${(report.subscribers.emailSubscribed - report.subscribers.emailUnsubscribed).toLocaleString()}</strong> net</span><span style="color:var(--muted)">·</span><span><strong>${report.subscribers.pushSubscribed.toLocaleString()}</strong> push subscribed</span><span><strong>${report.subscribers.pushUnsubscribed.toLocaleString()}</strong> unsubscribed</span><span><strong>${(report.subscribers.pushSubscribed - report.subscribers.pushUnsubscribed).toLocaleString()}</strong> net</span></div><div style="display:flex;flex-wrap:wrap;gap:1.2rem;margin-bottom:.7rem;font-size:.9rem;color:var(--muted)"><span>${report.subscribers.emailBounced.toLocaleString()} bounced</span><span>${report.subscribers.emailComplained.toLocaleString()} complained</span><span>${report.subscribers.emailOpened.toLocaleString()} opened</span><span>${report.subscribers.emailClicked.toLocaleString()} clicked</span><span>${report.subscribers.pushDelivered.toLocaleString()} push delivered</span><span>${report.subscribers.pushClicked.toLocaleString()} push clicked</span></div>${(() => { if (!report.subscribers.daily.length) return `<p style="color:var(--muted)">No subscriber activity in this period.</p>`; const max = Math.max(1, ...report.subscribers.daily.map(d=> Math.max(d.emailSubscribed,d.pushSubscribed))); const bars = report.subscribers.daily.map(d=> `<span class="metric-bar" style="height:${Math.max(2, Math.round(Math.max(d.emailSubscribed,d.pushSubscribed)/max*100))}%;background:var(--accent)" data-tooltip="${esc(d.date)}: +${d.emailSubscribed} email / +${d.pushSubscribed} push" aria-label="${esc(d.date)}" role="img" tabindex="0"></span>`).join(""); const rows = report.subscribers.daily.slice(-10).reverse().map(d=> `<tr><td>${esc(d.date)}</td><td class="num">+${d.emailSubscribed}</td><td class="num">-${d.emailUnsubscribed}</td><td class="num">+${d.pushSubscribed}</td><td class="num">-${d.pushUnsubscribed}</td></tr>`).join(""); return `<div class="metric-chart" aria-label="Daily subscriber joins">${bars}</div><table class="metrics" style="margin-top:.8rem"><thead><tr><th>Date</th><th class="num">Email +</th><th class="num">Email −</th><th class="num">Push +</th><th class="num">Push −</th></tr></thead><tbody>${rows}</tbody></table>`; })()}</div>
