...
// One global topbar for every authenticated admin page: constrained measure,
// brand, account cluster (Blogs, Billing, Affiliate, API), day/night toggle,
// and a mobile drawer. Blog pages add the context toolbar below it.
function accountNavLinks(linkClass: string, affiliateCurrent: string, billingCurrent: string): string {
  const cls = linkClass ? ` class="${linkClass}"` : "";
  return `<a${cls} href="/admin?list=1">Blogs</a>`
    + `<a${cls} href="/admin/billing"${billingCurrent}>Billing</a>`
    + `<a${cls} href="/admin/affiliate"${affiliateCurrent}>Affiliate</a>`
    + `<a${cls} href="/admin/api-key">API</a>`;
}

function accountTopbar(account: Account, planBadge: string, affiliateCurrent: string, billingCurrent: string): string {
  return `<header class="topbar globalbar owner-topbar">
        <div class="owner-topbar-inner">
          ...
          <div class="owner-account">
            ...
            ${accountNavLinks("", affiliateCurrent, billingCurrent)}
...
function accountDrawer(account: Account, paid: boolean, affiliateCurrent: string, billingCurrent: string, blogSection: string): string {
  return `<div class="owner-drawer-backdrop" id="owner-drawer-backdrop"></div>
      <aside class="owner-drawer" id="owner-drawer" aria-label="Owner menu" aria-hidden="true" inert>
        ...
        ${blogSection}
        <div class="owner-drawer-section"><div class="owner-drawer-label">Account</div>${accountNavLinks("owner-drawer-link", affiliateCurrent, billingCurrent)}${themeToggleButton(true)}<form method="post" action="/admin/logout"><button class="linkbtn" type="submit">Log out</button></form></div>
      </aside>`;
}
...
export function shell(
  ...
  if (account && tenant) {
    ...
    bar = `${accountTopbar(account, planBadge, affiliateCurrent, billingCurrent)}
      ...
      ${accountDrawer(account, paid, affiliateCurrent, billingCurrent,
    ...
    // Account-level pages (blog list, new blog, billing, affiliate) share the
    // same compact topbar; they just have no blog context toolbar.
    bar = `${accountTopbar(account, planBadge, affiliateCurrent, billingCurrent)}
      ${accountDrawer(account, paid, affiliateCurrent, billingCurrent, "")}`;
  }
  const switcherPart = account && tenant ? `
...
` : "";
  // Mobile drawer wiring ships on every authenticated admin page; the blog
  // switcher part above only exists on blog pages.
  const menuScript = account ? `<script>
    (function () {${switcherPart}
...
export function metricsPage(
  ...
  if (options?.configured === false) {
    ...
    const commentRows = report.comments.pages.map((item) => `<tr><td><a href="https://${esc(publicHost)}${esc(item.path)}" target="_blank" rel="noopener noreferrer">${esc(item.path)}</a></td><td class="num">${item.posted.toLocaleString()}</td><td class="num">${item.votes.toLocaleString()}</td></tr>`);
    const commentsPart = (() => {
      if (!commentRows.length) return `<tr><td colspan="3" style="color:var(--muted)">No comments yet.</td></tr>`;
      const { visible, hidden } = toRows(10, commentRows);
      return `${visible}${hidden ? `<tbody class="metrics-more-panel" hidden data-metrics-panel>${hidden}</tbody>` : ""}`;
    })();
    const commentsMore = commentRows.length > 10 ? `<button class="btn ghost metrics-more" type="button" data-metrics-more aria-expanded="false">Show ${commentRows.length - 10} more</button>` : "";
    const engagementChart = (() => {
      if (!report.comments.daily.length) return `<p style="color:var(--muted);font-size:.85rem">No engagement yet.</p>`;
      const maxEngaged = Math.max(1, ...report.comments.daily.map((day) => day.posted + day.votes));
      return `<div class="metric-chart" aria-label="Daily comment engagement">${
        report.comments.daily.map((day) => {
          const total = day.posted + day.votes;
          return `<span class="metric-bar" style="height:${Math.max(2, Math.round(total / maxEngaged * 100))}%" data-tooltip="${esc(day.date)}: ${total} engagements · ${day.posted} comments · ${day.votes} votes" aria-label="${esc(day.date)}: ${total} engagements, ${day.posted} comments, ${day.votes} votes" role="img" tabindex="0"></span>`;
        }).join("")
      }</div>`;
    })();
    ...
    <div class="metrics-grid">
      ...
      <div class="panel-block"><h2 style="margin-top:0;font-size:1rem">Comments</h2><div style="display:flex;gap:1.4rem;margin-bottom:.7rem"><span><strong>${report.comments.posted.toLocaleString()}</strong> comments</span><span><strong>${report.comments.votes.toLocaleString()}</strong> votes</span></div><div style="margin-bottom:.7rem"><strong style="font-size:.85rem">Daily engagement</strong>${engagementChart}</div><table class="metrics"><thead><tr><th>Post</th><th class="num">Comments</th><th class="num">Votes</th></tr></thead><tbody>${commentsPart}</tbody></table>${commentsMore}</div>
      <div class="panel-block"><h2 style="margin-top:0;font-size:1rem">Audience</h2><div style="display:flex;flex-wrap:wrap;gap:1.2rem;margin-bottom:.7rem"><span><strong>${report.subscribers.emailTotal.toLocaleString()}</strong> email subscribers</span><span style="color:var(--muted)">·</span><span><strong>${report.subscribers.pushTotal.toLocaleString()}</strong> push subscribers</span></div><div style="display:flex;flex-wrap:wrap;gap:1.2rem;margin-bottom:.7rem;font-size:.85rem;color:var(--muted)"><span>+${report.subscribers.emailSubscribed.toLocaleString()} email this period</span><span>-${report.subscribers.emailUnsubscribed.toLocaleString()} unsubscribed</span><span style="color:var(--muted)">·</span><span>+${report.subscribers.pushSubscribed.toLocaleString()} push this period</span><span>-${report.subscribers.pushUnsubscribed.toLocaleString()} unsubscribed</span></div>${(() => { if (!report.subscribers.daily.length) return `<p style="color:var(--muted)">No subscriber activity in this period.</p>`; const max = Math.max(1, ...report.subscribers.daily.map(d=> Math.max(d.emailSubscribed,d.pushSubscribed))); const bars = report.subscribers.daily.map(d=> `<span class="metric-bar" style="height:${Math.max(2, Math.round(Math.max(d.emailSubscribed,d.pushSubscribed)/max*100))}%;background:var(--accent)" data-tooltip="${esc(d.date)}: +${d.emailSubscribed} email / +${d.pushSubscribed} push" aria-label="${esc(d.date)}" role="img" tabindex="0"></span>`).join(""); const rows = report.subscribers.daily.slice(-10).reverse().map(d=> `<tr><td>${esc(d.date)}</td><td class="num">+${d.emailSubscribed}</td><td class="num">-${d.emailUnsubscribed}</td><td class="num">+${d.pushSubscribed}</td><td class="num">-${d.pushUnsubscribed}</td></tr>`).join(""); return `<div class="metric-chart" aria-label="Daily subscriber joins">${bars}</div><table class="metrics" style="margin-top:.8rem"><thead><tr><th>Date</th><th class="num">Email +</th><th class="num">Email −</th><th class="num">Push +</th><th class="num">Push −</th></tr></thead><tbody>${rows}</tbody></table>`; })()}</div>
      <div class="panel-block"><h2 style="margin-top:0;font-size:1rem">Notifications</h2><div style="display:flex;flex-wrap:wrap;gap:1.2rem;margin-bottom:.7rem"><span><strong>${report.subscribers.emailDelivered.toLocaleString()}</strong> email delivered</span><span><strong>${report.subscribers.emailBounced.toLocaleString()}</strong> bounced</span><span><strong>${report.subscribers.emailComplained.toLocaleString()}</strong> complained</span><span><strong>${report.subscribers.emailOpened.toLocaleString()}</strong> opened</span><span><strong>${report.subscribers.emailClicked.toLocaleString()}</strong> clicked</span></div><div style="display:flex;flex-wrap:wrap;gap:1.2rem;margin-bottom:.7rem"><span><strong>${report.subscribers.pushDelivered.toLocaleString()}</strong> push delivered</span><span><strong>${report.subscribers.pushClicked.toLocaleString()}</strong> push clicked</span></div>${(report.subscribers.emailDelayed || report.subscribers.emailDeliveryFailed || report.subscribers.emailHeld || report.subscribers.domainDnsError) ? `<div style="display:flex;flex-wrap:wrap;gap:1.2rem;margin-bottom:.7rem;font-size:.85rem;color:var(--muted)"><span>${report.subscribers.emailDelayed.toLocaleString()} delayed</span><span>${report.subscribers.emailDeliveryFailed.toLocaleString()} failed</span><span>${report.subscribers.emailHeld.toLocaleString()} held</span><span>${report.subscribers.domainDnsError.toLocaleString()} DNS error</span></div>` : ``}${(() => { const ndaily = report.subscribers.daily.filter(d=> d.emailDelivered || d.emailBounced || d.emailOpened || d.emailClicked || d.pushDelivered || d.pushClicked); if (!ndaily.length) return `<p style="color:var(--muted)">No notification activity in this period.</p>`; const rows = ndaily.slice(-10).reverse().map(d=> `<tr><td>${esc(d.date)}</td><td class="num">${d.emailDelivered}</td><td class="num">${d.emailBounced}</td><td class="num">${d.emailOpened}</td><td class="num">${d.emailClicked}</td><td class="num">${d.pushDelivered}</td><td class="num">${d.pushClicked}</td></tr>`).join(""); return `<table class="metrics" style="margin-top:.6rem"><thead><tr><th>Date</th><th class="num">Email delivered</th><th class="num">Bounced</th><th class="num">Opened</th><th class="num">Clicked</th><th class="num">Push delivered</th><th class="num">Push clicked</th></tr></thead><tbody>${rows}</tbody></table>`; })()}</div>
      ...
      (function () {
        ...
        autoFormat.addEventListener("click", function () {
          ...
          fetch("${base}/format-markdown", {
            ...
            autoFormatStatus.innerHTML = (data.unchanged ? 'No formatting changes were needed.' : (data.warning ? data.warning : 'Markdown formatting added. Review it before saving.')) + ' <button class="btn ghost" type="button" id="undo-auto-format">Undo auto-format</button>';
  ...
  return shell(
    ...
    `<div class="page">
      ...
      <form method="post" action="${base}/settings">
        ...
        <p style="color:var(--muted);font-size:.85rem;margin:-.5rem 0 1.2rem">Add up to 10 topics, separated by commas (1–3 is plenty). They help group blogs with similar themes; the first six appear publicly.</p>
