...
export function parseNavigationLinks(tenant: Tenant): NavigationLink[] {
  try {
    const raw = JSON.parse(tenant.navigation_links_json || "[]");
...
const STYLES = /* css */ `
  ...
  * { box-sizing: border-box; }
  ...
  .site-controls-more { display:none; width:2.35rem; height:2.35rem; align-items:center; justify-content:center; border:1px solid var(--rule); border-radius:999px; background:color-mix(in srgb,var(--bg) 90%,transparent); color:var(--muted); cursor:pointer; font:inherit; font-size:1.15rem; line-height:1; box-shadow:0 2px 8px rgb(0 0 0 / .08); }
  .site-controls-more:hover, .site-controls-more:focus-visible { color:var(--accent); border-color:var(--accent); }
  .site-controls-panel { display:contents; }
  .site-controls-panel[hidden] { display:contents !important; }
  .site-controls-more-wrap { display:contents; }
  ...
  .blog-subscribe-wrap { padding:2rem 0; }
  ...
  @media (max-width: 640px) {
    ...
    .blog-subscribe-wrap { padding:1.5rem 0; }
  }
  ...
  .share-more[aria-expanded="true"]::after, .share-more[aria-expanded="true"]:hover::after, .share-more[aria-expanded="true"]:focus-visible::after { opacity: 0 !important; transform: translate(-50%, .2rem) !important; }
  ...
  .post-audio-mobile { display: none; align-items: center; justify-content: center; width: 2.35rem; height: 2.35rem; border: 1px solid var(--rule); border-radius: 999px; background: var(--bg); color: var(--muted); cursor: pointer; flex: 0 0 auto; margin-left: auto; }
  .post-audio-mobile:hover, .post-audio-mobile:focus-visible { color: var(--accent); border-color: var(--accent); }
  .post-audio-mobile svg { width: 1rem; height: 1rem; fill: currentColor; }
  ...
  .subscribe {
    ...
    padding: 1.6rem 1.8rem; margin: 2rem 0 0.5rem;
  ...
  @media (max-width: 640px) {
    ...
    .post-audio { display: none !important; }
    .post-audio-mobile { display: inline-flex; }
    ...
    .site-controls-more { display:inline-flex; }
    .site-controls-more-wrap { display:flex; align-items:center; position:relative; }
    .site-controls-panel { display:flex; position:absolute; top:calc(100% + .5rem); right:0; z-index:12; min-width:10rem; padding:.45rem; background:var(--bg); border:1px solid var(--rule); border-radius:10px; box-shadow:0 12px 30px rgb(0 0 0 / .14); flex-direction:column; align-items:stretch; gap:.3rem; }
    .site-controls-panel[hidden] { display:none !important; }
    .site-controls-panel .rss-global, .site-controls-panel .rss-global a, .site-controls-panel .subscribe-link, .site-controls-panel .theme-toggle { width:100%; }
    .site-controls-panel .rss-global a, .site-controls-panel .subscribe-link, .site-controls-panel .theme-toggle { justify-content:flex-start; height:auto; padding:.55rem .7rem; border-radius:7px; border:none; background:transparent; box-shadow:none; font-size:.9rem; }
    .site-controls-panel .rss-global a:hover, .site-controls-panel .subscribe-link:hover, .site-controls-panel .theme-toggle:hover { background:color-mix(in srgb,var(--accent) 10%,transparent); color:var(--accent); }
    .homepage-wrap .blog-nav { padding-right:3.5rem; }
  }
`;
...
export function renderPost(
  ...
  const article = `<article class="post-page">
    ...
    <div class="byline">
      ...
      ${post.audio_key ? `<audio class="post-audio" data-narration controls preload="none" aria-label="Listen to this article" src="/media/${esc(post.audio_key)}">Your browser does not support audio playback.</audio><button class="post-audio-mobile" type="button" data-narration-mobile aria-label="Play narration"><svg class="icon-play" viewBox="0 0 24 24" aria-hidden="true"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg><svg class="icon-pause" viewBox="0 0 24 24" aria-hidden="true" hidden><rect x="6" y="4" width="4" height="16" rx="1"></rect><rect x="14" y="4" width="4" height="16" rx="1"></rect></svg></button>` : ""}
    </div>
    ${featuredBlock}
