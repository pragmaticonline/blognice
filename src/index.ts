...
import { AI_MARKDOWN_TEXT_MAX, markdownFormattingMessages, markdownFormattingRetryMessages, normalizedMarkdownResponse, preservesAuthorTokens } from "./ai-markdown";
  ...
  try {
    ...
    if (!preservesAuthorTokens(text, markdown)) {
      const retry = await c.env.AI.run(AI_BRIEF_MODEL, {
        messages: markdownFormattingRetryMessages(text, markdown), max_tokens: 6000, temperature: 0,
      });
    ...
    return c.json({
      error: changedWording
        ? "AI could not format this draft without changing its wording. Try again or format a shorter section. Your draft has not been changed."
        : "Auto-format failed. Please try again. Your draft has not been changed.",
    }, 502);
  }
});
