...
type Bindings = {
  ...
  ASSETS: R2Bucket; // public assets (press kit, etc.)
...
function expandYoutubeEmbeds(html: string): string {
  // Placeholder from markdown.ts: <div class="youtube-embed" data-youtube-id="VIDEOID"></div> (may have dataYoutubeId variant)
  const re = /<div class="youtube-embed"[^>]*data-youtube-id="([A-Za-z0-9_-]{11})"[^>]*><\/div>/g;
  const re2 = /<div class="youtube-embed"[^>]*dataYoutubeId="([A-Za-z0-9_-]{11})"[^>]*><\/div>/g;
  let out = html;
  const replacer = (full: string, vid: string) => {
    const safe = vid.replace(/[^A-Za-z0-9_-]/g, "");
    const src = `https://www.youtube-nocookie.com/embed/${safe}?rel=0&modestbranding=1&playsinline=1`;
    return `<div class="youtube-embed"><div class="youtube-embed__inner"><iframe src="${src}" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen loading="lazy" referrerpolicy="strict-origin-when-cross-origin" frameborder="0"></iframe></div></div>`;
  };
  out = out.replace(re, replacer as any);
  out = out.replace(re2, replacer as any);
  // Also handle legacy case where markdown left <p><a href="youtube..."> inside still? fallback: direct youtube link on its own line not converted (should not happen)
  return out;
}
...
app.get("/og-image.jpg", () => {
  ...
  return new Response(bytes, { headers: { "content-type": OG_IMAGE_CONTENT_TYPE, "content-length": String(bytes.length), "cache-control": "public, max-age=86400" } });
});
...
app.post("/admin/b/:blogId/format-markdown", async (c) => {
  ...
  try {
    ...
    const markdown = conservativeMarkdownFallback(text);
    queueBlogAudit(c, ctx.tenant.id, ctx.account.id, "markdown_formatted_with_fallback", `characters:${text.length}`);
    return c.json({
      markdown,
      warning: changedWording
        ? "AI changed the wording, so Blognice used safe basic formatting instead. No AI credit was used. Review it before saving."
        : "AI was temporarily unavailable, so Blognice used safe basic formatting instead. No AI credit was used. Review it before saving.",
    });
  }
});
...
function legalPage(c: any, page: string): Response {
  ...
  const canonicalFooter = '<footer class="footer"><span>© 2026 blognice · Pragmatic Online Co., Ltd.</span><nav class="footer-links" aria-label="Legal"><a href="/privacy">Privacy</a><a href="/terms">Terms</a><a href="/cookies">Cookies</a><a href="/security">Security</a><a href="/privacy#analytics-dialog">Analytics preferences</a></nav></footer>';
  const withCanonicalShell = consistentTheme.replace(/<footer class="footer">[\s\S]*?<\/footer>/, canonicalFooter);
  const withFounderTerms = page === termsPage
    ? withCanonicalShell.replace('<h2>4. Your content', '<p><strong>Founder pricing:</strong> US$5 per month, billed monthly, or US$36 per year, billed annually, plus applicable taxes. Your subscription renews at the price and billing interval selected for as long as it remains continuously active. If you cancel, allow it to lapse, or switch to another plan, the founder rate ends; a later subscription uses the then-current price. We may offer different prices to later subscribers.</p><h2>4. Your content')
    : withCanonicalShell;
  const withPrivacyPreferences = page === privacyPage
    ? withFounderTerms.replace("</body>", '<script>if(location.hash==="#analytics-dialog"){var analyticsDialog=document.getElementById("analytics-dialog");if(analyticsDialog)analyticsDialog.hidden=false}</script></body>')
    : withFounderTerms;
...
async function serveAssetFromR2(c: any, key: string, filename: string, contentType?: string) {
  const obj = await c.env.ASSETS.get(key);
  if (!obj) return c.notFound();
  const headers = new Headers();
  obj.writeHttpMetadata(headers);
  // @ts-ignore
  if (obj.httpEtag) headers.set("etag", obj.httpEtag as string);
...
app.get("/blognice-press-kit.zip", (c) => serveAssetFromR2(c, "blognice-press-kit.zip", "blognice-press-kit.zip", "application/zip"));
app.get("/press/blognice-press-kit.zip", (c) => serveAssetFromR2(c, "blognice-press-kit.zip", "blognice-press-kit.zip", "application/zip"));
app.get("/blognice-press-kit-v2.zip", (c) => serveAssetFromR2(c, "blognice-press-kit-v2.zip", "blognice-press-kit-v2.zip", "application/zip"));
app.get("/press/blognice-press-kit-v2.zip", (c) => serveAssetFromR2(c, "blognice-press-kit-v2.zip", "blognice-press-kit-v2.zip", "application/zip"));
...
function billingPage(
  ...
  const trialEligibleForPage = !(Number((billing as any)?.trial_used_at || 0) > 0);
  const trialNote = !active && trialEligibleForPage
    ? `<p class="billing-notice">Try Pro free for ${TRIAL_PERIOD_DAYS} days. Add a card today, pay nothing now — your plan begins only if you stay past day ${TRIAL_PERIOD_DAYS}. Cancel anytime.</p>`
    : "";
  ...
  // Keep literal for test: billing-ribbon">Founding price
  ...
  </style></head><body><main class="billing-wrap"><a class="billing-crumb" href="/admin">← blognice admin</a><h1 class="billing-h1">Billing</h1><p class="billing-account">Account: <b>${esc(account.email)}</b></p>${statusNotice}${unknownPaid}${usage}<section class="billing-section billing-plan-section"><div class="billing-section-head"><h2>Plan</h2>${active && renewal ? `<span>${esc(renewal)}</span>` : cryptoActive ? `<span>Valid through ${esc(cryptoExpiry)}</span>` : ""}</div><p class="billing-notice billing-founder-note"><strong>Founding member pricing</strong><br>Join Blognice while we're still building and help shape what comes next. <b>$36/year or $5/month.</b> Founding members keep their selected price for as long as that subscription remains continuously active. We plan to close founding pricing after the first 1,000 paying members. <strong>Planned standard pricing:</strong> $119/year or $12.99/month.</p>${trialNote}${billingAction ? `<div class="billing-main-action">${billingAction}</div>` : ""}<div class="billing-plans">${freeCard}${monthlyCard}${yearlyCard}</div>${cryptoOption}</section>${referralCode}<p class="billing-footnote">Stripe remains the primary payment provider. Payment details, receipts, invoices, cancellations, and plan changes are managed securely in Stripe. Crypto payments are annual-only, prepaid, and handled by NOWPayments; there is no automatic renewal. ${active ? "" : "Upgrade when you’re ready; access starts only after the payment provider confirms payment."}</p>${formerCustomer}</main></body></html>`;
}
...
function affiliatePerformanceChart(series: AffiliateFunnelDay[] | null): string {
  if (!series) return `<section class="affiliate-card"><h2>Clicks and sales</h2><p class="affiliate-card-copy">Last 90 days</p><p class="affiliate-chart-empty">Analytics are temporarily unavailable. Your exact sales and commission totals above are unaffected.</p></section>`;
  const width = 900, height = 260, left = 44, right = 16, top = 18, bottom = 34;
  const plotWidth = width - left - right, plotHeight = height - top - bottom;
  const peak = Math.max(1, ...series.flatMap((day) => [day.clicks, day.sales]));
  const x = (index: number) => left + (series.length <= 1 ? 0 : index * plotWidth / (series.length - 1));
  const y = (value: number) => top + plotHeight - value * plotHeight / peak;
  const points = (key: "clicks" | "sales") => series.map((day, index) => `${x(index).toFixed(1)},${y(day[key]).toFixed(1)}`).join(" ");
  const grid = [0, .25, .5, .75, 1].map((ratio) => {
    const gridY = top + plotHeight - ratio * plotHeight;
    return `<line x1="${left}" y1="${gridY}" x2="${width - right}" y2="${gridY}"/><text x="${left - 8}" y="${gridY + 4}" text-anchor="end">${Math.round(peak * ratio)}</text>`;
  }).join("");
  const labelIndexes = [...new Set([0, Math.floor((series.length - 1) / 2), series.length - 1])];
  const dateLabels = labelIndexes.map((index) => `<text x="${x(index)}" y="${height - 8}" text-anchor="${index === 0 ? "start" : index === series.length - 1 ? "end" : "middle"}">${esc(series[index]?.date || "")}</text>`).join("");
  const clickTotal = series.reduce((sum, day) => sum + day.clicks, 0);
  const saleTotal = series.reduce((sum, day) => sum + day.sales, 0);
  const range = series.length ? `${series[0].date} through ${series[series.length - 1].date}` : "the last 90 days";
  return `<section class="affiliate-card"><div class="affiliate-chart-head"><div><h2>Clicks and sales</h2><p class="affiliate-card-copy">Last 90 days · approximate, consent-aware analytics</p></div><div class="affiliate-chart-legend" aria-hidden="true"><span class="clicks">Clicks ${clickTotal}</span><span class="sales">Sales ${saleTotal}</span></div></div><div class="affiliate-chart-wrap"><svg class="affiliate-chart" viewBox="0 0 ${width} ${height}" role="img" aria-labelledby="affiliate-chart-title affiliate-chart-desc"><title id="affiliate-chart-title">Referral clicks and sales over 90 days</title><desc id="affiliate-chart-desc">${clickTotal} link clicks and ${saleTotal} sales from ${esc(range)}.</desc><g class="affiliate-chart-grid">${grid}${dateLabels}</g><polyline class="affiliate-chart-line clicks" points="${points("clicks")}"/><polyline class="affiliate-chart-line sales" points="${points("sales")}"/></svg></div><p class="affiliate-help">Clicks can be lower than actual traffic where analytics consent is required or tracking is blocked. Sales are provider-confirmed conversion events.</p></section>`;
}

function affiliateDashboardPage(account: Account, dashboard: AffiliateDashboard | null, rootDomain: string, terms: AffiliateTermsConfig | null, message = "", performance: AffiliateFunnelDay[] | null = null): string {
  const styles = `<style>
    .affiliate-page{max-width:1040px}.affiliate-head{display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;margin-bottom:1.5rem}.affiliate-head h1{margin:0 0 .3rem}.affiliate-eyebrow{color:var(--muted);margin:0}.affiliate-status{display:inline-flex;align-items:center;min-height:2rem;padding:.35rem .7rem;border-radius:999px;background:color-mix(in srgb,var(--accent) 12%,transparent);color:var(--accent);font-size:.8rem;font-weight:750}.affiliate-card{background:var(--panel);border:1px solid var(--rule);border-radius:14px;padding:1.4rem;margin:0 0 1rem}.affiliate-card h2{font-size:1.1rem;margin:0 0 .35rem}.affiliate-card-copy{color:var(--muted);margin:.2rem 0 1rem}.affiliate-alert{border:1px solid #e8c58c;background:#fff8e8;border-radius:12px;padding:1rem 1.1rem;margin-bottom:1rem}.affiliate-alert h2{font-size:1rem;margin:0 0 .3rem}.affiliate-alert p{margin:.2rem 0 .8rem}.affiliate-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem}.affiliate-enrollment-layout{display:grid;grid-template-columns:minmax(0,1.15fr) minmax(19rem,.85fr);gap:1.25rem;align-items:start}.affiliate-facts{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.75rem;margin-top:1.2rem}.affiliate-fact,.affiliate-kpi{border:1px solid var(--rule);border-radius:11px;padding:1rem;background:var(--bg)}.affiliate-fact strong,.affiliate-kpi strong{display:block;font-size:1.45rem;line-height:1.15}.affiliate-fact span,.affiliate-kpi span{display:block;color:var(--muted);font-size:.82rem;margin-top:.25rem}.affiliate-form{display:grid;gap:.8rem}.affiliate-form label{font-weight:650}.affiliate-form input[type=text],.affiliate-form select,.affiliate-referral-input{width:100%;min-height:44px;border:1px solid var(--rule);border-radius:9px;background:var(--field);color:var(--ink);padding:.7rem .8rem;font:inherit}.affiliate-check{display:grid;grid-template-columns:auto 1fr;gap:.65rem;align-items:start;font-weight:400!important}.affiliate-check input{width:1.1rem;height:1.1rem;margin-top:.2rem}.affiliate-btn{min-height:44px;border:1px solid var(--accent);border-radius:9px;padding:.65rem 1rem;background:var(--accent);color:var(--accent-ink);font:inherit;font-weight:700;cursor:pointer}.affiliate-btn.secondary{background:var(--panel);color:var(--accent)}.affiliate-referral-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:.6rem}.affiliate-referral-input{font-family:var(--mono);overflow:hidden;text-overflow:ellipsis}.affiliate-code{display:inline-flex;padding:.3rem .55rem;border-radius:6px;background:var(--bg);font:700 .9rem var(--mono)}.affiliate-kpi-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.75rem}.affiliate-kpi.primary{background:color-mix(in srgb,var(--accent) 9%,var(--panel));border-color:color-mix(in srgb,var(--accent) 30%,var(--rule))}.affiliate-kpi.primary strong{color:var(--accent)}.affiliate-chart-head{display:flex;align-items:flex-start;justify-content:space-between;gap:1rem}.affiliate-chart-legend{display:flex;gap:1rem;flex-wrap:wrap;font-size:.82rem;font-weight:700}.affiliate-chart-legend span:before{content:"";display:inline-block;width:.75rem;height:.18rem;border-radius:99px;margin-right:.35rem;vertical-align:.2rem}.affiliate-chart-legend .clicks:before{background:var(--accent)}.affiliate-chart-legend .sales:before{background:#b45309}.affiliate-chart-wrap{overflow-x:auto}.affiliate-chart{display:block;width:100%;min-width:38rem;height:auto}.affiliate-chart-grid line{stroke:var(--rule);stroke-width:1}.affiliate-chart-grid text{fill:var(--muted);font:11px system-ui,sans-serif}.affiliate-chart-line{fill:none;stroke-width:3;stroke-linecap:round;stroke-linejoin:round;vector-effect:non-scaling-stroke}.affiliate-chart-line.clicks{stroke:var(--accent)}.affiliate-chart-line.sales{stroke:#b45309}.affiliate-chart-empty{color:var(--muted);padding:2rem 0}.affiliate-table-wrap{overflow-x:auto}.affiliate-table{width:100%;min-width:34rem}.affiliate-table th{color:var(--muted);font-size:.75rem;text-transform:uppercase;letter-spacing:.04em}.affiliate-empty{text-align:center;padding:2rem!important;color:var(--muted)}.affiliate-badge{display:inline-flex;padding:.22rem .55rem;border-radius:999px;background:var(--bg);font-size:.78rem;font-weight:700}.affiliate-help{color:var(--muted);font-size:.85rem}.affiliate-copy-status{min-height:1.2rem;color:var(--accent);font-size:.82rem;margin:.45rem 0 0}
...
app.get("/admin/affiliate", async (c) => {
  ...
  let performance: AffiliateFunnelDay[] | null = null;
  if (dashboard && metricsConfigured(c.env)) {
    try {
      performance = await affiliateFunnelSeries(c.env, account.id, 90);
    } catch (error) {
      console.error(JSON.stringify({ message: "Affiliate analytics query failed", accountId: account.id, error: error instanceof Error ? error.message.slice(0, 300) : "Unknown analytics error" }));
    }
  }
  return c.html(affiliateDashboardPage(account, dashboard, c.env.ROOT_DOMAIN, terms, String(c.req.query("message") || "").slice(0, 200), performance));
});
...
app.post("/admin/billing/checkout", async (c) => {
  ...
  let billing: any;
  try {
    billing = await c.env.DB.prepare("SELECT stripe_customer_id, billing_status, vip_granted_at, vip_expires_at, trial_used_at FROM accounts WHERE id = ?").bind(account.id).first<{ stripe_customer_id?: string | null; billing_status: string; vip_granted_at?: number | null; vip_expires_at?: number | null; trial_used_at?: number | null }>();
  } catch {
    try {
  ...
  // One free trial per account: a stamped trial_used_at means a trial was
  // already granted, including via cancel-then-resubscribe.
  const trialEligible = !(billing && Number((billing as any).trial_used_at || 0) > 0);
  ...
  try {
    ...
    const trialDays = trialEligible ? TRIAL_PERIOD_DAYS : null;
...
app.post("/stripe/webhook", async (c) => {
  ...
  if (event.type.startsWith("customer.subscription.") && accountId && subscriptionObject) {
    ...
    const subscriptionUpdate = await c.env.DB.prepare("UPDATE accounts SET stripe_customer_id = COALESCE(?, stripe_customer_id), stripe_subscription_id = ?, billing_subscription_created_at = COALESCE(?, billing_subscription_created_at), billing_status = ?, trial_used_at = CASE WHEN ? = 'trialing' THEN COALESCE(trial_used_at, ?) ELSE trial_used_at END, billing_price_id = ?, billing_period_end = ?, billing_cancel_at_period_end = ?, billing_updated_at = ?, billing_event_created_at = ?, billing_event_id = ? WHERE id = ? AND (COALESCE(billing_event_created_at, 0) < ? OR (COALESCE(billing_event_created_at, 0) = ? AND COALESCE(billing_event_id, '') < ?))")
      .bind(customerId, subscriptionObject.id || object.id || null, subscriptionObject.created || null, status, status, Math.floor(Date.now() / 1000), subscriptionObject.items?.data?.[0]?.price?.id || null, periodEnd, subscriptionObject.cancel_at_period_end ? 1 : 0, Math.floor(Date.now() / 1000), eventCreated, event.id, accountId, eventCreated, eventCreated, event.id).run();
    await c.env.DB.prepare("UPDATE stripe_events SET account_id = ? WHERE id = ?").bind(accountId, event.id).run();
        ...
        if (shouldQueue) {
          ...
          try {
            await c.env.EMAIL_QUEUE.send({
              ...
              idempotencyKey,
              to: account.email,
              ...subscriptionActiveEmail({ billingUrl, plan: subscriptionObject.items?.data?.[0]?.price?.id === c.env.STRIPE_YEARLY_PRICE_ID ? "yearly" : subscriptionObject.items?.data?.[0]?.price?.id === c.env.STRIPE_MONTHLY_PRICE_ID ? "monthly" : undefined, trialDays: status === "trialing" ? TRIAL_PERIOD_DAYS : null }),
...
// future comments carry the photo. Photos appear instantly, matching the
// blog's auto-approve posture.
const COMMENT_AVATAR_MAX = 2 * 1024 * 1024;
app.post("/:slug/comments/avatar", async (c) => {
  const tenant = await resolveTenant(c.env, c.req.header("host") || "");
  if (!tenant || !tenant.comments_enabled) return c.json({ error: "Comments are not available." }, 404);
  const cookie = getCookie(c, COMMENT_COOKIE);
  if (!cookie) return c.json({ error: "Verify your email address before commenting." }, 401);
  ...
  if (!identity || !identity.verified_at) return c.json({ error: "Verify your email address before commenting." }, 401);
  let form: FormData;
  try { form = await c.req.formData(); } catch { return c.json({ error: "Expected multipart/form-data with an avatar field." }, 400); }
  ...
  if (!ALLOWED_IMAGE.has(file.type) || (await detectedImageType(file)) !== file.type) return c.json({ error: "Unsupported image type." }, 400);
  if (file.size > COMMENT_AVATAR_MAX) return c.json({ error: "Profile photos must be 2 MB or smaller." }, 413);
  const rand = crypto.randomUUID().slice(0, 8);
  const key = `avatars/${tenant.id}-${identity.email_hash.slice(0, 8)}-${rand}.${EXT[file.type]}`;
  const old = typeof identity.avatar_key === "string" && identity.avatar_key.startsWith("avatars/") ? identity.avatar_key : null;
  await c.env.MEDIA.put(key, await file.arrayBuffer(), { httpMetadata: { contentType: file.type, cacheControl: "public, max-age=31536000, immutable" } });
  await tenantDb(c.env, tenant).prepare(
...
app.delete("/:slug/comments/avatar", async (c) => {
  const tenant = await resolveTenant(c.env, c.req.header("host") || "");
  if (!tenant || !tenant.comments_enabled) return c.json({ error: "Comments are not available." }, 404);
  const cookie = getCookie(c, COMMENT_COOKIE);
  if (!cookie) return c.json({ error: "Verify your email address before commenting." }, 401);
