...
app.get("/blognice-press-kit.zip", async (c) => {
app.get("/press/blognice-press-kit.zip", async (c) => {
  ...
  return new Response(bytes as unknown as BodyInit, { headers: { "content-type": "application/zip", "content-disposition": 'attachment; filename="blognice-press-kit.zip"', "cache-control": "public, max-age=3600" } });
});
