...
function legalPage(c: any, page: string): Response {
  ...
  const consistentTheme = page
    ...
    .replace('</nav><a href="/" class="meta">', `</nav>${policyNavMarkup}<a href="/" class="meta">`)
  ...
  const withPrivacyPreferences = page === privacyPage
    ? withCanonicalShell.replace("</body>", '<script>if(location.hash==="#analytics-dialog"){var analyticsDialog=document.getElementById("analytics-dialog");if(analyticsDialog)analyticsDialog.hidden=false}</script></body>')
    : withCanonicalShell;
