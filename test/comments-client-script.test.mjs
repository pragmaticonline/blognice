...
test("served comment scripts parse and keep their regex escapes", async () => {
  ...
  assert.ok(client.includes("dialog.contains(form)"), "subscribe row shows only with the form in the modal");
  assert.ok(client.includes("draftSub"), "verify-restore carries the modal subscribe decision");
