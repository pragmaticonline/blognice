...
CREATE TABLE tenants (
  ...
  comments_enabled INTEGER NOT NULL DEFAULT 0, -- owner opt-in for blog comments (see migrations/070)
...
-- Autopilot: per-blog automated publishing configs and runs (061)
...
-- OAuth 2.1 Authorization Code + PKCE for ChatGPT / Claude MCP (062)
...
CREATE INDEX idx_oauth_codes_account ON oauth_authorization_codes(account_id, expires_at);
CREATE INDEX idx_oauth_codes_client ON oauth_authorization_codes(client_id, expires_at);
