...
The automated A/A allocation fixture verifies an exact 5,000/5,000 split over all 10,000 allocation buckets. Provider-seam tests cover Stripe and delayed/duplicate NOWPayments facts. The staff page is available at `/staff/experiments/affiliate-offer` behind Cloudflare Access and labels D1 totals exact and Analytics Engine trends approximate. Exact decision diagnostics include provider checkout failures, refunded conversions and revenue, exclusion totals, Affiliate representation, and largest-Affiliate exposure share; staff must judge material differences rather than relying on an automated winner declaration.

Activation is intentionally a later operational decision. Staff Analytics Engine credentials and the successful empty-trends state were verified in production while dormant. Before changing the flag, an authorized owner must still approve the copy and privacy/retention treatment, freeze the baseline, detectable uplift, sample target and dates in D1, and run a live payment-attribution canary. Deployment of the dormant implementation is not activation.
