...
- Staff Analytics Engine trends remain unavailable until a scoped Account Analytics Read token is installed as `CF_ANALYTICS_TOKEN` on the `blognice-staff` Worker. Runtime workflow `33780597297` reverified the dormant routes and the draft/empty production experiment, then failed only at this missing-secret gate. Exact D1 totals and decision diagnostics remain available; do not activate before this prerequisite is resolved and verified.
