...
- Migration 055 hardens presentation immutability after exposure and adds D1/Stripe/NOWPayments checkout snapshots of experiment key and variant. It was applied and verified in production by workflow run `33779702543`; pre-migration Time Travel bookmark: `0000193c-00000000-000050db-d4e2317589dbf4f5d9b2715074144102`.
- A scoped Account Analytics Read token is installed as `CF_ANALYTICS_TOKEN` on the `blognice-staff` Worker. Runtime workflow `33781500924` passed the dormant-route, draft/empty D1, and secret-presence gates; an authenticated production page check then returned the successful empty-trends state rather than the explicit Analytics error state. Exact D1 totals and decision diagnostics are also available.
- Public HTTP coverage validates enrollment and active dashboard states. The complete suite passed 347/347; typecheck, diff check, and both production Worker dry-runs passed.
